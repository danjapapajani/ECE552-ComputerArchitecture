...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  //get the next line
  int cache_line_size = cp->bsize;
  md_addr_t next_line = addr + cache_line_size; 
  //make sure it's aligned
  //divide by cache_line_size
  //multiply by cache_line_size
  if (next_line % cache_line_size != 0){
    next_line = next_line / cache_line_size;
    next_line = next_line * cache_line_size;
  }
  //check if it's in the cache before trying to prefetch
  int in_cache_already = cache_probe(cp, next_line);
  if (!in_cache_already){
...
/****** OPENENDED PREFETCHER ********/ 
/* ECE552 Assignment 4 - BEGIN CODE */
typedef enum {Initial, Transient, Steady, NoPrediction} State;
typedef struct oe_rpt_entry {
  md_addr_t tag;
  md_addr_t prev_addr;
  int stride_1;
  State state_1;
...
int oe_initialized = 0;

oe_rpt_entry *oe_rpt;

#define OE_RPT_SIZE 7000

/* Open Ended Prefetcher */
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
	if (!oe_initialized){
    oe_rpt = (oe_rpt_entry *)malloc(OE_RPT_SIZE * sizeof(oe_rpt_entry));
    for (int i = 0; i < OE_RPT_SIZE; i++){
      oe_rpt[i].tag = 0;
      oe_rpt[i].prev_addr = 0;
      oe_rpt[i].stride_1 = 0;
      oe_rpt[i].stride_2 = 0;
      oe_rpt[i].stride_3 = 0;
      oe_rpt[i].stride_4 = 0;
      oe_rpt[i].state_1 = Initial;
      oe_rpt[i].state_2 = Initial;
      oe_rpt[i].state_3 = Initial;
      oe_rpt[i].state_4 = Initial;
    }
    oe_initialized = 1;
  ...
  md_addr_t pc = get_PC();
  //discard lowest 3 bits because it's a 64-bit address
  int negative_oe_rpt_size = -1 * OE_RPT_SIZE;
  unsigned int mask = (~negative_oe_rpt_size) << 3;
  md_addr_t index = (pc & mask) >> 3; 

  //Scenario 1
  if (oe_rpt[index].tag == 0){
    oe_rpt[index].tag = pc;
    oe_rpt[index].prev_addr = addr;
    oe_rpt[index].stride_1 = 0;
    oe_rpt[index].stride_2 = 0;
    oe_rpt[index].stride_3 = 0;
    oe_rpt[index].stride_4 = 0;
    oe_rpt[index].state_1 = Initial;
    oe_rpt[index].state_2 = Initial;
    oe_rpt[index].state_3 = Initial;
    oe_rpt[index].state_4 = Initial;
  }

  //Scenario 2
  //there is a corresponding entry
  else {
    int new_stride = addr - oe_rpt[index].prev_addr;
    oe_rpt[index].tag = pc;
    oe_rpt[index].prev_addr = addr;
    int stride;
    State state;
    ...
    for (int i = 0; i < 4; i++){ 
      switch (i){
        case 0:
          stride = oe_rpt[index].stride_1;
          state = oe_rpt[index].state_1;
          break;
        case 1:
          stride = oe_rpt[index].stride_2;
          state = oe_rpt[index].state_2;
          break;
        case 2:
          stride = oe_rpt[index].stride_3;
          state = oe_rpt[index].state_3;
          break;
        
        case 3:
          stride = oe_rpt[index].stride_4;
          state = oe_rpt[index].state_4;
          break;
        
      }

      //stride condition is true
      if (new_stride == stride){
        if (i == 0){
          if (state == NoPrediction){
            oe_rpt[index].state_1 = Transient;
          }
          else {
            oe_rpt[index].state_1 = Steady;
          }
        }
        else if (i == 1){
          if (state == NoPrediction){
            oe_rpt[index].state_2 = Transient;
          }
          else {
            oe_rpt[index].state_2 = Steady;
          }
        }
        else if (i == 2){
          if (state == NoPrediction){
            oe_rpt[index].state_3 = Transient;
          }
          else {
            oe_rpt[index].state_3 = Steady;
          }
        }
        
        else {
          if (state == NoPrediction){
            oe_rpt[index].state_4 = Transient;
          }
          else {
            oe_rpt[index].state_4 = Steady;
          }
        }
        
      }

      //stride condition is false
      else{
        if (i == 0){
          switch (oe_rpt[index].state_1){
            case Initial:
              oe_rpt[index].state_1 = Transient;
              oe_rpt[index].stride_1 = new_stride;
              break; 
            case Transient:
              oe_rpt[index].state_1 = NoPrediction;
               oe_rpt[index].stride_1 = new_stride;
              break;
            case Steady:
              oe_rpt[index].state_1 = Initial;
          ...
          switch (oe_rpt[index].state_2){
            case Initial:
              oe_rpt[index].state_2 = Transient;
              oe_rpt[index].stride_2 = new_stride;
              break; 
            case Transient:
              oe_rpt[index].state_2 = NoPrediction;
               oe_rpt[index].stride_2 = new_stride;
              break;
            case Steady:
              oe_rpt[index].state_2 = Initial;
          ...
          switch (oe_rpt[index].state_3){
            case Initial:
              oe_rpt[index].state_3 = Transient;
              oe_rpt[index].stride_3 = new_stride;
              break; 
            case Transient:
              oe_rpt[index].state_3 = NoPrediction;
               oe_rpt[index].stride_3 = new_stride;
              break;
            case Steady:
              oe_rpt[index].state_3 = Initial;
          ...
          switch (oe_rpt[index].state_4){
          case Initial:
            oe_rpt[index].state_4 = Transient;
            oe_rpt[index].stride_4 = new_stride;
            break; 
          case Transient:
            oe_rpt[index].state_4 = NoPrediction;
              oe_rpt[index].stride_4 = new_stride;
            break;
          case Steady:
            oe_rpt[index].state_4 = Initial;
      ...
      if (i == 0){
        if (oe_rpt[index].state_1 == Steady){

          md_addr_t prefetch_addr = addr + oe_rpt[index].stride_1; 
          //align it to the size of the cache block
          if (prefetch_addr % cp->bsize != 0){
            prefetch_addr = prefetch_addr / cp->bsize;
            prefetch_addr = prefetch_addr * cp->bsize;
          }

        
          //check that it's not already in the cache
          int in_cache_already = cache_probe(cp, prefetch_addr);
          if (!in_cache_already){
        ...
        if (oe_rpt[index].state_2 == Steady){

          md_addr_t prefetch_addr = addr + oe_rpt[index].stride_2; 
          //align it to the size of the cache block
          if (prefetch_addr % cp->bsize != 0){
            prefetch_addr = prefetch_addr / cp->bsize;
            prefetch_addr = prefetch_addr * cp->bsize;
        ...
        if (oe_rpt[index].state_3 == Steady){

            md_addr_t prefetch_addr = addr + oe_rpt[index].stride_3; 
            //align it to the size of the cache block
            if (prefetch_addr % cp->bsize != 0){
              prefetch_addr = prefetch_addr / cp->bsize;
              prefetch_addr = prefetch_addr * cp->bsize;
        ...
        if (oe_rpt[index].state_4 == Steady){

            md_addr_t prefetch_addr = addr + oe_rpt[index].stride_4; 
            //align it to the size of the cache block
            if (prefetch_addr % cp->bsize != 0){
              prefetch_addr = prefetch_addr / cp->bsize;
              prefetch_addr = prefetch_addr * cp->bsize;
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  if (!initialized){
    //initialize the RPT, where the number of entries is the prefetch type
    rpt = (rpt_entry *)malloc(cp->prefetch_type * sizeof(rpt_entry));

    //initialize every rpt_entry in the rpt to be 0
  ...
  md_addr_t pc = get_PC();
  //discard lowest 3 bits because it's a 64-bit address
  int negative_rpt_size = -1 * cp->prefetch_type;
  unsigned int mask = (~negative_rpt_size) << 3;
  md_addr_t index = (pc & mask) >> 3; 
  ...
  //there is a corresponding entry
    ...
    //stride condition is true
    if (new_stride == rpt[index].stride){
      if (rpt[index].state == NoPrediction){
    ...
    // only make a prediction if the rpt has a prediction
    if (rpt[index].state != NoPrediction){

      md_addr_t prefetch_addr = addr + rpt[index].stride; 
      //align it to the size of the cache block
      if (prefetch_addr % cp->bsize != 0){
        prefetch_addr = prefetch_addr / cp->bsize;
        prefetch_addr = prefetch_addr * cp->bsize;
